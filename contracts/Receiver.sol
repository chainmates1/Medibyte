...
import {EnumerableMap} from "@chainlink/contracts-ccip@1.4.0/src/v0.8/vendor/openzeppelin-solidity/v4.8.3/contracts/utils/structs/EnumerableMap.sol";
...
contract Receiver is CCIPReceiver, OwnerIsCreator {
    ...
    error CallToHealthContractFailed(); // Used when the call to the selectTests function of the Health contract is not succesful
    ...
    address private immutable i_Health_Contract;
    ...
    /// @param _Health_Contract The address of the Health_Contract contract.
    constructor(
        address _router,
        address _usdcToken,
        address _Health_Contract
    ) CCIPReceiver(_router) {
        if (_usdcToken == address(0)) revert InvalidUsdcToken();
        i_usdcToken = IERC20(_usdcToken);
        i_Health_Contract = _Health_Contract;
        i_usdcToken.approve(_Health_Contract, type(uint256).max);
    }
    ...
    function _ccipReceive(
        ...
        (bool success, ) = i_Health_Contract.call(
            any2EvmMessage.data
        ); // low level call to the Health contract using the encoded function selector and arguments
        if (!success) revert CallToHealthContractFailed();
