...
const fetchNFTDataFromOpenSea = async (tokenId) => {
  const response = await fetch(
    `https://api.opensea.io/api/v1/asset/${PatientNFT_ADDRESS}/${tokenId}/`
  );
...
const ProfileCard = () => {
  ...
  const RewardLink = () => {
    navigate("/Rewards", {
      state: {
        tokens: nftData.attributes.find((attr) => attr.trait_type === "Tokens")
          .value,
        score: nftData.attributes.find((attr) => attr.trait_type === "Score")
          .value,
        id: TokenId,
      },
    });
