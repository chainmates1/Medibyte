...
const Rewards = () => {
    ...
    const claimReward = async (rewardKey) => {
        try {
            ...
            const rewardPrice = rewardContracts[rewardKey].price;

            if (balance.lt(ethers.utils.parseUnits(rewardPrice.toString(), 18))) {
                throw new Error("Insufficient token balance");
            }

            // Approve tokens
            const approveTx = await medicoin.approve('YOUR_HEALTH_CONTRACT_ADDRESS', ethers.utils.parseUnits(rewardPrice.toString(), 18));
            ...
            // Update NFT data
            const updatedTokens = balance.sub(ethers.parseUnits(rewardPrice.toString(), 18));
